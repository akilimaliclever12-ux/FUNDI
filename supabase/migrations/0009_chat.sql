...
--
-- IMPORTANT (Supabase SQL Editor): run STEP 1 ALONE first, then STEP 2.
-- `alter type ... add value` cannot run inside a transaction block, so it must
-- be executed by itself; otherwise the whole script aborts.

-- ============================================================
-- STEP 1 — run this line by itself, then run STEP 2 below
-- ============================================================
alter type lead_channel add value if not exists 'chat';

-- ============================================================
-- STEP 2 — run everything below (idempotent, safe to re-run)
-- ============================================================
...
-- Realtime for message inserts (guarded so re-runs don't error)
do $$ begin
  alter publication supabase_realtime add table public.messages;
exception when duplicate_object then null; end $$;

-- Tell PostgREST to refresh its API schema cache
